<?
// original version: https://github.com/Zverik/mbtiles-php
...
if( isset($_GET['tileset']) && preg_match('/^[\w\d_-]+$/', $_GET['tileset']) ) {
	...
	if (strlen($tileset) > 5 && substr($tileset,strlen($tileset)-5) == '-wmts') {
		$tileset = substr($tileset,0,strlen($tileset)-5);
        $layout = 'wmts';
		$flip = false;
	}
	try {
        $file = $tileset.'.mbtiles';
        if ($layout == 'wmts') {
            $dim = '';
            for ($i = 0; $i < 5; $i++) {
                if (isset($_GET['d'.$i])) {
            ...
            $file = '1.0.0/'.$tileset.'/'.$_GET['style'].'/'.$dim.$_GET['grid'].'.'.$_GET['ext'].'.mbtiles';
        }
        $db = new PDO('sqlite:'.$file,'','',array(PDO::ATTR_PERSISTENT => true));
		if( !isset($db)) {
