...
int main(int argc, char* argv[])
{
  ...
  if (argc < 3)
  {
    ...
    cout << "color-shade <input_dem> <input_color_scale> <output_relief_map> "
...
//      }
      ...
      if (i == 0)
      {
        if (j == 0)
        {
          poBand->RasterIO( GF_Read, j, i, 2, 2,
               win, 2, 2, GDT_Float32,
               0, 0 );
          win[2][2] = win[3];
          win[2][1] = win[2];
          win[1][2] = win[1];
          win[1][1] = win[0];
          win[0][0] = 2*win[1][1] - win[2][2];
          win[0][1] = 2*win[1][1] - win[2][1];
          win[0][2] = 2*win[1][2] - win[2][2];
          win[1][0] = 2*win[1][1] - win[1][2];
          win[2][0] = 2*win[2][1] - win[2][2];
        }
        else if (j == nXSize-1)
        {
          poBand->RasterIO( GF_Read, j-1, i, 2, 2,
               win, 2, 2, GDT_Float32,
               0, 0 );
          win[1][2] = win[3];
          win[1][1] = win[2];
          ...
          win[1][0] = 2*win[1][1] - win[1][2];
          win[2][0] = 2*win[1][1] - win[2][2];
          win[2][1] = 2*win[1][1] - win[0][1];
          win[2][2] = 2*win[1][2] - win[0][2];
        }
        else
        {
          poBand->RasterIO( GF_Read, j-1, i, 3, 2,
               win, 3, 2, GDT_Float32,
               0, 0 );
          win[2][2] = win[5];
          win[2][1] = win[4];
      ...
      else if (i == nYSize-1)
      {
        if (j == 0)
        {
          poBand->RasterIO( GF_Read, j, i-1, 2, 2,
               win, 2, 2, GDT_Float32,
               0, 0 );
          win[2][1] = win[3];
          win[2][0] = win[2];
          ...
          win[0][1] = 2*win[1][1] - win[2][1];
          win[0][2] = 2*win[1][1] - win[2][0];
          win[1][2] = 2*win[1][1] - win[1][0];
          win[2][2] = 2*win[2][1] - win[2][0];
        }
        else if (j == nXSize-1)
        {
          poBand->RasterIO( GF_Read, j-1, i-1, 2, 2,
               win, 2, 2, GDT_Float32,
               0, 0 );
          win[1][1] = win[3];
          win[1][0] = win[2];
          win[0][1] = win[1];
          win[0][0] = win[0];
          win[0][2] = 2*win[0][1] - win[0][0];
          win[1][2] = 2*win[1][1] - win[1][0];
          win[2][0] = 2*win[1][0] - win[0][0];
          win[2][1] = 2*win[1][1] - win[0][1];
          win[2][2] = 2*win[1][1] - win[0][0];
        }
        else
        {
          poBand->RasterIO( GF_Read, j-1, i-1, 3, 2,
               win, 3, 2, GDT_Float32,
               0, 0 );
          win[2][1] = win[5];
          win[2][0] = win[4];
      ...
      else
      {
        if (j == 0)
        {
          poBand->RasterIO( GF_Read, j, i-1, 2, 3,
               win, 2, 3, GDT_Float32,
               0, 0 );
          win[2][2] = win[5];
          win[2][1] = win[4];
        ...
        {
          poBand->RasterIO( GF_Read, j-1, i-1, 2, 3,
               win, 2, 3, GDT_Float32,
               0, 0 );
          win[1][2] = win[5];
          win[1][1] = win[4];
      ...
      if (shadeBuf[j] == nullValue && j != 0) {
        ...
        float facteur = 0.5 + shadeBuf[j] / 256.0;
